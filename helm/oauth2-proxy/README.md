...
| `startupProbe.enabled`                              | enable Kubernetes startupProbe. Disable to use oauth2-proxy with Istio mTLS. See [Istio FAQ](https://istio.io/help/faq/security/#k8s-health-checks)                                                                                                            | `false`                                                                                               |
| `startupProbe.exec`                                   | command to execute in the container              | ``                           |
| `startupProbe.httpGet`                                | HTTP GET request to perform                      | `{}`                           |
| `startupProbe.tcpSocket`                              | TCP socket to connect to                         | `{}`                           |
| `startupProbe.grpc`                                   | GRPC health check request                        | `{}`                           |
| `startupProbe.initialDelaySeconds`                    | seconds to wait before starting probes           | ``                            |
| `startupProbe.periodSeconds`                          | seconds between each probe                       | `10`                           |
| `startupProbe.timeoutSeconds`                         | probe timeout in seconds                         | `5`                            |
| `startupProbe.failureThreshold`                       | number of failures before marking as failed      | `3`                            |
| `startupProbe.successThreshold`                       | number of successes before marking as successful | `1`                            |
| `startupProbe.terminationGracePeriodSeconds`          | seconds to terminate gracefully on failure       | ``            |
